...
However, this may not be very helpful for control system engineers since lot of their flow graphs have closed loops and hardcoding the blocks to make it open loop is counter-intutive. Thus, a separate scheduler called GNU Radio Advanced Scheduler is being developed to incorporate closed loop flow graphs. It uses C++ libaries such as Theron(concurrency library), PMC(Polymorphic Message Container for C++ library) etcetra.
...
\begin{figure}[ht!]
	...
	\caption{Continuous time simulation flow graph}
	\label{overflow}
\end{figure}
...
\begin{figure}[ht!]
	...
	\caption{Discrete time simulation flow graph}
	\label{overflow}
\end{figure}
